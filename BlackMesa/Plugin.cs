...
namespace BlackMesa
{
    ...
    public class BlackMesaInterior : BaseUnityPlugin
    {
        ...
        public void CheckDoorways(DungeonFlow dungeonFlow)
        {
            ...
            foreach (Tile dungeonTile in dungeonFlow.GetTiles())
            {
                foreach (Doorway dungeonDoorway in dungeonTile.gameObject.GetComponentsInChildren<Doorway>())
                {
                    ...
                    {
                        if (doorwayTileWeight == null)
                            mls.LogInfo($"GameObjectWeight was null on doorway \"{GetPath(dungeonDoorway.transform)}\".");
                        if (doorwayTileWeight.GameObject == null)
                            mls.LogInfo($"GameObjectWeight.GameObject was null on doorway \"{GetPath(dungeonDoorway.transform)}\".");
                    }
                }
            }
        }
